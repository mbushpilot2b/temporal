	...
	t.Run("Update completes", func(t *testing.T) {
		ts.Run(t, func(s *Scenario) {
			...
			Act(wkr, RespondWorkflowTaskCompleted{
				Task: wft,
				Commands: Select(s, "workflow response",
					Option("non-empty task", GenList(ScheduleActivityTaskCommand{TaskQueue: tq})),
					Option("empty task", []Arbitrary[*commandpb.Command]{})),
			})
